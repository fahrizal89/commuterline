    ...
    fun find(startNode: Int, endNode:Int): Route {
        ...
        for (cnt in 0 until totalVertex - 1) {
            ...
            for (vx in 0 until totalVertex) {
                ...
                val nodeDistance :Int = graph[ux].route?.get(vxKey)?.dis ?: -1

                if (!spSet[vx]!! && graph[ux].route?.get(vxKey)?.dis != -1 && graph[ux].route?.get(vxKey) != null && distance[ux] != Int.MAX_VALUE && distance[ux] + nodeDistance < distance[vx]) {
